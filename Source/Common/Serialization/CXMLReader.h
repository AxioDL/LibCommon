...
public:
    ...
    bool PreSerializePointer(void*& InPointer, uint32_t Flags) override
    {
        return mpCurElem->GetText() == nullptr || strcmp(mpCurElem->GetText(), "NULL") != 0;
    }

    void SerializePrimitive(bool& rValue, uint32_t Flags)  override    { rValue = ReadParam() == "true"; }
    void SerializePrimitive(char& rValue, uint32_t Flags) override     { rValue = ReadParam().Front(); }
    void SerializePrimitive(int8_t& rValue, uint32_t Flags) override   { rValue = (int8_t)ReadParam().ToInt32((Flags & SH_HexDisplay) ? 16 : 0); }
    void SerializePrimitive(uint8_t& rValue, uint32_t Flags) override  { rValue = (uint8_t)ReadParam().ToInt32((Flags & SH_HexDisplay) ? 16 : 0); }
    void SerializePrimitive(int16_t& rValue, uint32_t Flags) override  { rValue = (int16_t)ReadParam().ToInt32((Flags & SH_HexDisplay) ? 16 : 0); }
    void SerializePrimitive(uint16_t& rValue, uint32_t Flags) override { rValue = (uint16_t)ReadParam().ToInt32((Flags & SH_HexDisplay) ? 16 : 0); }
    void SerializePrimitive(int32_t& rValue, uint32_t Flags) override  { rValue = (int32_t)ReadParam().ToInt32((Flags & SH_HexDisplay) ? 16 : 0); }
    void SerializePrimitive(uint32_t& rValue, uint32_t Flags) override { rValue = (uint32_t)ReadParam().ToInt32((Flags & SH_HexDisplay) ? 16 : 0); }
    void SerializePrimitive(int64_t& rValue, uint32_t Flags) override  { rValue = (int64_t)ReadParam().ToInt64((Flags & SH_HexDisplay) ? 16 : 0); }
    void SerializePrimitive(uint64_t& rValue, uint32_t Flags) override { rValue = (uint64_t)ReadParam().ToInt64((Flags & SH_HexDisplay) ? 16 : 0); }
    void SerializePrimitive(float& rValue, uint32_t Flags) override    { rValue = ReadParam().ToFloat(); }
    void SerializePrimitive(double& rValue, uint32_t Flags) override   { rValue = (double)ReadParam().ToFloat(); }
    void SerializePrimitive(TString& rValue, uint32_t Flags) override  { rValue = ReadParam(); }
    void SerializePrimitive(CFourCC& rValue, uint32_t Flags) override  { rValue = CFourCC(ReadParam()); }
    void SerializePrimitive(CAssetID& rValue, uint32_t Flags) override { rValue = CAssetID::FromString(ReadParam()); }
